...
app.post('/chat', async (req, res) => {
  ...
  try {
    ...
    if (message.toLowerCase() === 'yes' && userSession.hasAskedConfirmation) {
      ...
      rowData.push(
        userSession.transcript.map((entry) => `${entry.role}: ${entry.content}`).join(' | ')
      );
    ...
    if (userSession.hasScheduledAppointment && !userSession.hasAskedConfirmation) {
      ...
      userSession.hasAskedConfirmation = true;
    ...
    // Detect if the bot has scheduled an appointment
    if (response.toLowerCase().includes('we have booked you') || response.toLowerCase().includes('appointment scheduled')) {
      userSession.hasScheduledAppointment = true;
    }
